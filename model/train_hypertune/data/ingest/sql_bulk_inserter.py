...
#         """
#         Convert a list of dictionaries to CSV format using StringIO.
        
#         Args:
#             data (list): List of dictionaries. If header is None, the keys of the
#                          first dictionary will be used as the CSV header.
#             header (str or list, optional): Comma-separated string or list representing
#                         the CSV header. If provided, these keys will be used.
...
#         # Write header row
#         output.write(",".join(header_keys) + "\n")
#         # Write each row using the header order
#         for row in data:
#             # Use .get() so that if a key is missing in a row, it will default to an empty string.
#             row_values = [str(row.get(key, "")).replace(',', ';') for key in header_keys]
...
#                                              If not provided, keys from the first
#                                              dictionary are used.
#         """
#         if not data:
...
#         # Convert the dictionary data to CSV format using StringIO
...
#         )
#         cur = conn.cursor()
#         copy_sql = f"COPY {table}({columns}) FROM STDIN WITH CSV HEADER"
